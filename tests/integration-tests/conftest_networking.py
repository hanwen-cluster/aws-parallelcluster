...
# Split the VPC address space into 22 CIDRs. Each public subnet has 1024 addresses. Each private subnet or custom subnet has 4096 addresses
...
def get_az_id_to_az_name_map(region, credential, zone_types=None):
    ...
    return {entry.get("ZoneId"): entry.get("ZoneName") for entry in describe_availability_zones(region, credential, **kwargs)}

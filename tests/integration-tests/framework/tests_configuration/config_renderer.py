...
def _check_or_create_capacity_reservations(config_file, os_parameters, instance_type_parameters):
    ...
    for var in variables:
        if "CAPACITY_RESERVATION" in var:
            ...
            if len(specs) == 1:  # Single instance type: use original logic
                ...
                if _find_and_modify_existing_capacity_reservation(
                    ...
                    continue
            if not _create_multi_reservations(az_for_capacity_reservation, candidate_regions, specs, var):
                az_for_capacity_reservation[var] = "use1-az6"
        return az_for_capacity_reservation


def _create_multi_reservations(az_for_cr, regions, specs, var):
            ...
            for az in ec2_client.describe_availability_zones()["AvailabilityZones"]:
                ...
                for instance_type, os_platform, count, end_date, enable_placement_group in specs:
                    reservation_id = _create_single_capacity_reservation(
                        zone_id, count, ec2_client, enable_placement_group, end_date, instance_type, os_platform
                    )
                    if reservation_id:
...
def _create_single_capacity_reservation(
    ...
    try:
        ...
        reservation_id = ec2_client.create_capacity_reservation(**reservation_args)["CapacityReservation"]["CapacityReservationId"]

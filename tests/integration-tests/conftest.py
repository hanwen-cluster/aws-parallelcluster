...
@pytest.fixture()
def hardened_tmp_ami_factory(region, vpc_stack, request, cfn_stacks_factory):
    """Build temporary AMIs whose /tmp mount is noexec from early boot."""
    template_path = (
        Path(__file__).parent / "tests" / "createami" / "test_createami" / "test_build_image" / "hardened-tmp-ami.yaml"
    )
